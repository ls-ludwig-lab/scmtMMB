...
mtVar.bulk.df <- mtVar.bulk.df %>% 
  mutate(occurrence = case_when(variant %in% mtVar_parental ~ "Parental",
                                ...
                                TRUE ~ "Private")) %>% 
  mutate(occurrence = factor(occurrence, levels = c("Parental", "WT-shared" ,"POLG-shared","Private")))
...
lapply(c("CTRL", "KI36","KIA2"), function(cdt){
  ...
  print(p5)
  ggsave(plot = p5, paste0("../plot/25_heteroplasmy_bulk_occur_",cdt,".pdf"), width = 4, height = 3)
})
...
lapply(c("Parental", "Private","POLG-shared"), function(occu){
