...
HEK <- readRDS("../POLG_HEK_large_data_files/output/11_POLG_HEK_preprocesssed_seurat.rds")
...
saveRDS(mitoSE.CTRL, "../POLG_HEK_large_data_files/output/02_mgatk.CTRL.rds")
saveRDS(mitoSE.KI36, "../POLG_HEK_large_data_files/output/02_mgatk.KI36.rds")
saveRDS(mitoSE.KIA2, "../POLG_HEK_large_data_files/output/02_mgatk.KIA2.rds")
```
...
mtVar.bulk.df <- mtVar.bulk.df %>% mutate(occurrence = case_when(variant %in% c(mtVar_WT_only, mtVar_KI36_only, mtVar_KIA2_only) ~ "Private",
                                                                 variant %in% mtVar_parental ~ "Parental",
                                                                 variant %in% mtVar_POLG_sahred ~ "POLG-sahred"))
...
stat.test
write.csv(stat.test, file = "../output/2_het_stat.test.csv", 
          quote = FALSE, row.names = TRUE)

lapply(c("CTRL", "KI36","KIA2"), function(cdt){
  colors =  c("Parental" = "#7CAAFF", "POLG-sahred" = "#583690","Private" = as.character(color_vec[cdt]))
  p5 <- mtVar.bulk.df %>% filter(condition == cdt) %>% ggplot(aes(x = occurrence, y = mean)) +
  geom_violin(aes(fill = occurrence), width = 0.7) + geom_boxplot(width=0.1) + theme_classic() + 
  scale_fill_manual(values = colors) + scale_y_log10(limits = c(5e-5,1), breaks = 10^(-5:0)) +
  ggtitle(cdt) + Seurat::NoLegend()
  print(p5)
  ggsave(plot = p5, paste0("../plot/25_heteroplasmy_bulk_occur_",cdt,".pdf"), width = 3, height = 3)
})

stat.test.occu <- mtVar.bulk.df %>% select(condition, occurrence, mean) %>% 
  group_by(condition) %>% 
  t_test(mean ~ occurrence) %>%
  adjust_pvalue() %>%
  add_significance("p.adj")
stat.test.occu
write.csv(stat.test.occu, file = "../output/2_het_stat.test.occu.csv", 
          quote = FALSE, row.names = TRUE)

lapply(c("Parental", "Private","POLG-sahred"), function(occu){
  ...
  print(p6)
  ggsave(plot = p6, paste0("../plot/26_heteroplasmy_bulk_cdt_",occu,".pdf"), width = 3, height = 3)
})
...
stat.test.cdt <- mtVar.bulk.df %>% select(condition, occurrence, mean) %>% 
  ...
  add_significance("p.adj")
stat.test.cdt
write.csv(stat.test.cdt, file = "../output/2_het_stat.test.cdt.csv", 
          quote = FALSE, row.names = TRUE)
