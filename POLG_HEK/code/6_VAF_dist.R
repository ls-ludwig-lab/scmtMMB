...
disease.df <- mitomap.disease %>%
  mutate(variant = paste0(pos, ref, ">", alt)) %>%
  filter(status %in% c("Cfrm", "Cfrm [P*]", " Cfrm [LP*]")) 

# write.csv(disease.df, "../output/SuppTable_mitomap_disease.csv", quote = FALSE)
disease.var <- disease.df%>% pull(variant)
...
meta1 <- prepare_mtVAR_meta(mmat_list[[1]], mito.annotation)
...
df <- rbind(meta1, meta2, meta3)
write.csv(df, file = "../output/SupTable_POLG_variants.csv", quote = FALSE)
...
SO <- Seurat::AddMetaData(SO, metadata = as.data.frame(SO@reductions$atac.umap@cell.embeddings))
...
KIA2 <- subset(KIA2, atacUMAP_1 > 1.5)
...
save_variant_plots <- function(plots, prefix) {
  for (name in names(plots)) {
    file_path <- file.path("../plot/Fig3_VAF_dist_plots/", paste0(prefix, "_", name, ".pdf"))
    ggsave(
      filename = file_path,
      plot = plots[[name]],
      width = 5,
...
# Example usage
save_variant_plots(plots_KI36_c, "KI36")
save_variant_plots(plots_KIA2_c, "KIA2")
