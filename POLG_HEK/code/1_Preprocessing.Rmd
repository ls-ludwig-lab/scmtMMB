...
p1 <- RidgePlot(HEK, assay = "HTO", features = rownames(HEK[["HTO"]])[1:3], ncol = 3, cols = c(color_vec, "Doublet" = "black", "Negative" = "grey"))
ggsave(plot = p1, "../plot/10_HTO_demultiplex.pdf", width = 12, height = 4)
```
...
HEK <- subset(HEK, hash.ID != "Doublet")
HEK$condition <- data.frame(hash.ID = HEK$hash.ID, seurat_clusters = HEK$seurat_clusters) %>% 
                 reframe(condition = case_when(HEK$hash.ID == "CTRL" & HEK$seurat_clusters %in% c(1,6) ~ "CTRL",
                                         ...
                                         HEK$hash.ID == "Negative" & HEK$seurat_clusters %in% c(1,6) ~ "CTRL",
                                         HEK$hash.ID == "Negative" & HEK$seurat_clusters %in% c(3,4,5,7) ~ "KI36",
                                         HEK$hash.ID == "Negative" & HEK$seurat_clusters %in% c(0,2) ~ "KIA2",
...
write.csv(HEK@meta.data, file = "../output/1_HEK_metadata.csv", 
