...
HEK <- readRDS(file = "../POLG_HEK_large_data_files/output/1_POLG_HEK_preprocesssed_seurat.rds")
...
mmatMut.CTRL <- readRDS("output/2_mmtx.CTRL.rds")[CTRL_dnV,]
mmatMut.KI36 <- readRDS("output/2_mmtx.KI36.rds")[KI36_dnV,]
mmatMut.KIA2 <- readRDS("output/2_mmtx.KIA2.rds")[KIA2_dnV,]
...
p1 <- MutNum.summary %>% ggplot(aes(x = cutoff, y = mean + 1, color = condition)) +
    ...
    scale_y_log10(limits = c(1, 1500), expand = c(0,0),
...
p1
ggsave(plot = p1, "../plot/Fig2d_41_MMB_mean_cutoff_log.pdf", width = 3, height = 3)
...
p2 <- MutNum %>% tidyr::pivot_longer(cols = -c("condition", "atacUMAP_1", "atacUMAP_2", "mtDNA_depth"), 
  ...
  scale_colour_viridis_c(limits = c(0, 4)) +
...
p2.nl 
ggsave("../plot/Fig2e_42_mutNum_sample_cutoff_UMAP.pdf", p2.nl, width = 8, height = 4)
ggsave("../plot/Fig2e_42_mutNum_sample_cutoff_UMAP_ori.pdf", p2, width = 8, height = 4)
ggsave("../plot/Fig2e_42_leg.pdf", p2.leg, width = 2, height = 2)
...
p3.nl
ggsave(plot = p3.nl, "../plot/Ext_Fig2_43_depth_MMB_cutoff.pdf", width = 8, height = 4)
ggsave(plot = p3, "../plot/Ext_Fig2_43_depth_MMB_cutoff.ori.pdf", width = 8, height = 4)
