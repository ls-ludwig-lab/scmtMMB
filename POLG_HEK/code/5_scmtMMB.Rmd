...
- MSS 
```{r , message=FALSE, warning=FALSE, fig.width=9, fig.height=2}
...
ggsave(plot = p1, "../plot/51_MSS_complex.pdf", width = 14, height = 2.5)
```

- MSS weighted by heteroplasmy
```{r , message=FALSE, warning=FALSE, fig.width=7, fig.height=2}
(p2 <- scmtMMB_HEK_com %>% 
...
ggsave(plot = p3, "../plot/53_mutation_per_MB_complex.pdf", width = 14, height = 2.5)
...
scmtMMB_HEK %>% filter(symbol == "Total") %>% 
   ggplot(aes(x = mtDNA_depth, y = mutation_per_MB + 1)) +
  geom_point(aes(col = sample)) + facet_wrap(~sample) + 
  scale_color_manual(values = color_vec) + 
  theme_bw() + ylab("Mutation per MB (Total)") + theme(legend.position="none") +
  geom_smooth(method = "lm")
