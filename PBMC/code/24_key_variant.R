...
# scmtMMB <- read.csv("../PBMC_large_data_files/output/2_scmtMMB.csv", row.names = "X") %>% filter(barcode != "pseudobulk")
...
p1
ggsave(plot = p1, paste0("../plot/3_H05_ND4L.pdf"), width = 2, height = 2)
...
ggsave(plot = p2, paste0("../plot/3_H05_ND4L_G10559A.pdf"), width = 2, height = 2)
...
ggsave(plot = p2.2.leg, paste0("../plot/3_density.leg.pdf"), width = 3, height = 2)

ggsave(plot = p2.2.nl, paste0("../plot/3_H05_ND4L_G10559A_smooth_umap.pdf"), width = 2, height = 2)

# rador plot
above_10 <- df_H05 %>% filter(VAF != 0) %>% 
  dplyr::select(celltype, VAF) %>%
  dplyr::group_by(celltype) %>%
  dplyr::summarize(n = n(),
                   across(-n, ~sum(VAF > 0.1))) %>%
  ungroup()

data <- as.data.frame(rbind(rep(1,length(unique(celltype_mito_combined_filter$predicted.celltype.l1))) , 
                            rep(0,length(unique(celltype_mito_combined_filter$predicted.celltype.l1))) , 
                            t(test_radial_plot_above_0), 
                            t(test_radial_plot_above_10), 
                            t(test_radial_plot_above_60)))

fmsb::radarchart(data,  title= paste0("Radial Plot ", var))
...
p3
ggsave(plot = p3, paste0("../plot/3_M60_ND3.pdf"), width = 2, height = 2)
...
p4 <- df_M60 %>% 
  ...
  theme_classic()+ theme_void() + Seurat::NoLegend()
p4
ggsave(plot = p4, paste0("../plot/3_M60_ND3_T10270C.pdf"), width = 2, height = 2)
...
p2.2
ggsave(plot = p2.2, paste0("../plot/3_M60_ND3_T10270C_smooth_umap.pdf"), width = 2, height = 2)
...
p5 
ggsave(plot = p5 , paste0("../plot/3_M80_ND3.pdf"), width = 2, height = 2)
...
p6 <- df_M80 %>% 
  ...
  theme_classic() + theme_void() + Seurat::NoLegend()
p6
ggsave(plot = p6, paste0("../plot/3_M80_ND3_A10398G.pdf"), width = 2, height = 2)
...
p2.2 <- ggplot(df_M80 %>% arrange(af_weight),
  ...
  theme(legend.position = "none")
p2.2
ggsave(plot = p2.2, paste0("../plot/3_M80_ND3_A10398G_smooth_umap.pdf"), width = 2, height = 2)
...
p7
ggsave(plot = p7, paste0("../plot/3_M80_RNR2.pdf"), width = 2, height = 2)
...
p8.1 <- scmtMMB %>% filter(sample == "M80" & symbol == "Total") %>% 
  ...
  theme_classic()+ theme_void() + Seurat::NoLegend()
p8.1
ggsave(plot = p8.1, paste0("../plot/3_M80_RNR2_G1969A.pdf"), width = 2, height = 2)
...
p8.2 <- scmtMMB %>% filter(sample == "M80" & symbol == "Total") %>% 
  ...
  theme_classic()+ theme_void() + Seurat::NoLegend()
p8.2
ggsave(plot = p8.2, paste0("../plot/3_M80_RNR2_G3079A.pdf"), width = 2, height = 2)
